...
class _Victron extends BTSensor{
    ...
    getModelName(){
        return this.constructor.MODEL_ID_MAP[this.model_id]
    }
    decrypt(data){
        if (!this.advertisementKey)
    ...
    async connect() {
        if (this.advertisementKey){
            this.cb = async (propertiesChanged) => {
                try{
                    const data = await this.device.getManufacturerData()             
